...
15. Windows 自定义安装选择的目录始终作为父目录，无论是否为空都必须在其中创建产品专属子目录；默认安装和升级使用已经解析好的最终安装目录，不得额外嵌套。卸载脚本禁止对 `$INSTDIR` 使用递归或文档通配符删除，只能删除产品拥有的明确程序文件，最后以非递归方式尝试移除空目录；用户原有文件及软件在安装目录内新建的文档都必须保留。
...
6. 每个后续版本都必须在 Windows 程序每次启动、检查更新和执行更新时静默检查 `uninstall.exe`，不得依赖网络是否可用或更新提醒设置：带 `QUILLITE_SAFE_UNINSTALL_V1` 安全标识的版本必须原样保留且禁止删除、改名或禁用；只有注册表卸载命令精确指向当前程序目录中同一文件时，才允许把无安全标识的旧版判定为本产品拥有的风险卸载器。风险版优先只删除该精确文件，删除失败时改名禁用，再失败时只移除精确匹配的卸载登记；严禁仅凭文件名处理未知文件，也不得使用通配符、递归删除或扩大到安装目录。独立的重大缺陷弹框保持取消；只有上述方式全部无法解除风险时，才阻断后续热更新并回退为完整安装包。
