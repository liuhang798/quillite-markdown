...
6. Windows 启动时必须在联网检查更新前独立检查安装目录中的 `uninstall.exe` 安全标识，并对旧版、缺失或读取失败显示重大缺陷警告；该次启动不得紧接着重复弹出常规更新窗口。常规更新弹框必须隐藏热更新入口，只允许用户从官网下载安装包覆盖安装；后端下载接口也必须执行同样的阻断，不能只依赖前端按钮。
