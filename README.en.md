...
- Added a complete AI Assistant for DeepSeek, Zhipu GLM, Qwen, OpenAI, and Kimi, including live model discovery and a default provider-and-model choice. Provider API keys remain isolated in the native system credential vault and are never written to preferences, logs, or feedback data.
...
- Typora-style Mermaid diagrams render directly from fenced ` ```mermaid ` blocks. Flowcharts now open on a visual canvas where process, decision, and start/end nodes can be added, dragged, connected, relabelled, reshaped, and automatically arranged in four directions without editing source. Put the cursor inside an existing flowchart fence to reveal Edit flowchart on canvas and save changes back to that block in place. Only the drawing workspace expands in full-screen mode, where Exit and Insert/Save remain available and the canvas can be zoomed from 50% to 200% with buttons or `Ctrl/Cmd + wheel`. At every zoom level, empty-space dragging pans freely, edge dragging extends the view, and Select all or `Ctrl/Cmd + A` lets every node move as one group. Quillite still generates standard portable Mermaid; advanced subgraphs and styling can stay safely in Source mode. The other Mermaid templates retain editable source, live preview, and one-click insertion, while Word/HTML/PDF exports preserve the rendered result. [Open the Mermaid examples](docs/Mermaid-图表完整案例.md).
