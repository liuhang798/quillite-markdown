...
- 修复 macOS 点击编辑器“更多格式”需要等待 1–2 秒的问题；原生 WebKit 选择器已替换为应用内即时菜单。
- “更多格式”完整保留工具栏折叠项、学科公式、图表生成器和其他扩展格式，并支持方向键、Home／End 与 Escape 键盘操作。
- 修复新版 macOS SDK 下安全书签解码和释放可能崩溃的问题，最近阅读、收藏和资源浏览器可继续稳定恢复授权。
- 修复新版 macOS SDK 构建时缺少系统框架导致应用链接失败的问题，恢复 Apple Silicon 与 Universal 发布构建。
- 安全书签解析到系统规范路径后仍保留用户原始文档路径，避免最近记录出现路径变化或重复项。
