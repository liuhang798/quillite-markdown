...
> V2.7.4 重新构建：文档保存检测外部修改，冲突时保留编辑并要求另存；读取、保存与恢复快照统一上限为 64 MiB。Pandoc 导出失败不会覆盖原输出，远程导出图片不允许访问内网。特殊文件属性或目录权限无法安全处理时保留原文件，并提示另存副本。旧卸载器识别的已知限制见更新日志；跨平台安装/升级实机验证及正式证书签名仍未全部完成。已安装相同版本的用户需手动下载新安装包。
