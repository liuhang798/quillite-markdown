...
- 新增完整 AI 助手：支持 DeepSeek、智谱 GLM、通义千问、OpenAI 与 Kimi，可加载账号实际可用模型并设置默认“服务 + 模型”；各服务 API Key 分别保存在系统凭据库，不写入偏好、日志或反馈数据。
...
- 可选 AI 助手支持两种安全流程：“AI编辑”只处理用户主动选中的 Markdown，可润色、改写、精简、扩写、总结、翻译或执行自定义要求；“AI检查”在用户确认后检查整篇文档，逐条列出可精确定位的语法、拼写、标点、表达、一致性与 Markdown 建议，用户可勾选后一次性修改，并可通过一次撤销恢复。内置 DeepSeek、智谱 GLM、通义千问、OpenAI 与 Kimi 官方接口，保存对应 API Key 后会通过该服务的模型接口加载可用模型，也可在接口暂不可用时继续使用推荐模型。各服务的 Key 可同时存在并分开安全保存于 Windows 凭据管理器、macOS 钥匙串或 Linux Secret Service，默认服务与默认模型可组合选择，切换默认模型不会覆盖或删除其他 Key；Key 不写入偏好、日志或反馈数据，使用云端接口前会逐次确认是否发送文档内容。
