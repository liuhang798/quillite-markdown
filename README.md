...
- 支持 Typora 风格 Mermaid 图表：流程图默认提供可视化画布，无需修改源码即可直接添加处理、判断和开始／结束节点，拖动节点、点击连接、修改文字／形状／线条，并使用四种方向自动排列；光标进入已经插入的流程图代码块后，可点击“在画布中编辑流程图”再次打开并原位保存。可仅将绘图区一键铺满窗口，全屏时直接退出或插入／保存图表，并通过按钮或 `Ctrl/Cmd + 滚轮` 在 50%–200% 间缩放；任意倍率下均可拖动空白画布自由平移，拖至边缘自动延展视野，“全选”或 `Ctrl/Cmd + A` 后拖动任一节点可整体移动。软件会自动生成标准 Mermaid 源码，仍兼容其他 Markdown 编辑器。包含子图或样式等高级语法时可安全切回源码模式。其他 Mermaid 图表继续支持源码编辑、实时预览和一键插入，Word／HTML／PDF 导出保持预览排版。[查看 22 类 Mermaid 图表完整案例](docs/Mermaid-图表完整案例.md)。
