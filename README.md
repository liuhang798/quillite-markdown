...
AI 设置的服务与模型选择框采用一致的跨平台样式，按“服务与地址 → API Key → 模型”的顺序配置。输入服务地址和 API Key 后会自动查询账号可用模型，无需先保存 Key；查询使用输入值直接完成且不会把 Key 写入偏好。阿里云百炼业务空间会自动适配其专用模型列表接口。DeepSeek 未保存 Key 时显示内置候选，不代表账号实际权限；刷新失败会保留上次结果并提示原因。自定义 OpenAI 兼容接口不假定默认模型，仍需由用户选择自动加载的模型或手动填写实际模型 ID。
