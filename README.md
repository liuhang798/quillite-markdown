...
开发中：保存遇到外部修改时，可并排对比“磁盘版本／当前编辑”。保留编辑会暂停自动保存；使用磁盘版本只替换编辑器内容且可撤回；另存副本必须选择新文件名。手动合并先应用到编辑器，再次点击保存后明确确认，期间磁盘再次变化会重新进入对比，不自动覆盖。文件被删除或无法读取时仍保留当前编辑。

## 为什么选择轻阅 Markdown？

开发中安全修复：保存的最终提交不覆盖其他程序刚创建的目标；Windows 排他校验原文件，无法取得保护时按冲突处理。失败时恢复原文件或保留恢复材料。macOS/Linux 会在文档旁保留隐藏的 `.quillite-save-recovery-*` 原文件副本，以保住外部程序已有句柄的迟到写入；这些副本会占用磁盘，不自动清理。修改中的文件系统若不支持安全操作，保存会失败而不会降低保护等级，请另存新副本。

开发中：在“更多 → 文档工具”使用基础 Markdown 检查、全文搜索、批量替换预览、文档安全中心、模板和诊断导出。检查不改文；模板只新建文档；安全中心只读恢复备份与当前文档历史，恢复采用另存新副本。诊断仅导出版本、平台、状态布尔值和会话错误分类计数，不包含正文、路径或密钥，不上传。

全文搜索是区分大小写的纯文本磁盘搜索，支持相对路径和类型筛选；不搜索当前打开的文档，最多扫描 800 个文件、5 层目录和 32 MiB 文本，单文件 2 MiB、最多返回 500 行。批量替换每批最多 40 个文件、每文件 256 KiB；预览有效 10 分钟。必须逐项勾选并确认，保存历史失败、路径变为链接或修订冲突均跳过，不自动覆盖或重试。模板支持 `{{title}}`、`{{date}}`，最多 20 个自定义模板，每个 64 KiB。

冲突恢复保护：异常退出后仍按原始修订号判断是否允许保存；旧快照与未解决冲突需要再次对比确认。未解决冲突的备份不会因为撤回到旧内容而清除；过期合并草稿也不会直接应用到后续编辑。

保存回执仅对应本次写入的内容，保存后出现的外部修改仍需重新对比，不能被下一次自动保存覆盖。阅读模式下的手动合并也会更新恢复备份，不依赖是否打开过编辑器。

保存与 AI 编辑按文档会话隔离，避免延迟结果影响其他文件；保存过程中继续输入的内容仍保留未保存提示。内置范例另存后可作为普通文档编辑。

普通文档保存采用先写临时文件再替换的方式，写入失败时保留原文件。历史版本清理仅处理已验证归属的快照。Windows 安装和卸载会再次核验程序目录归属，不会凭快捷方式名称删除或覆盖用户链接；macOS 更新会保留上一版应用包供恢复。
...
编辑时可关闭实时预览进入“仅编辑”，编辑器占满可用宽度；点击“另存为”后面的“恢复预览”可恢复上次分栏方向。也可在“更多 → 编辑布局”选择“仅编辑”“编辑在左”或“预览在左”，软件会记住选择。按 `Ctrl + E`（Mac：`⌘ + E`）切换完整预览，再按一次返回编辑。隐藏实时预览时停止自动渲染并释放图表实例；恢复分栏后显示最新正文，保存及按需导出不受影响。
...
V2.7.5 中，旧卸载器必须同时通过已审核 SHA-256 风险名单与精确卸载登记核验才可自动处理。未知文件和登记原样保留；当前缺少已验证旧样本，风险名单为空，未标记文件不会自动删除或改名。无法确认安全时暂停热更新，提示备份后在新目录完整安装。

> V2.7.5：新增文档工具、安全的批量替换与冲突版本对比；新增全宽“仅编辑”布局和一键恢复预览。Windows 普通卸载入口改为经过严格所有权校验的原位运行，发布流水线验证安装、升级、卸载全过程中的用户文档保留。正式证书签名仍未配置。
...
Windows 用户运行 `quillite-markdown-版本-windows-amd64.exe` 即可；安装程序使用共用背景的简洁准备页、路径确认页、安装页与完成页，统一使用右上角关闭图标，所有可点击操作悬停时显示手型光标。安装流程默认显示简体中文，右下角可随时切换 English／中文，准备、确认、安装中、完成、失败、按钮、动态提示、错误信息与目录选择提示会同步切换且不会重置安装进度。全新安装会沿用安装器当前语言：使用 English 流程安装后软件默认进入英文，使用中文流程则默认进入简体中文；升级安装会保留原有语言偏好。准备页点击“下一步”或选择“自定义安装”文件夹后，都会进入确认页并显示最终安装路径；可返回或更改位置，只有点击“确认并安装”才会开始安装。安装中保留动态百分比，按钮文字循环显示“正在安装.／..／...”且不加载进度条或多层动画；安装核心成功后才显示 100%，完成页按钮为“完成安装”，点击可关闭安装器，未操作时也会在3秒后自动关闭。全新安装默认使用当前用户 Programs 目录；仅当旧目录同时存在主程序和有效专属标识时，升级才沿用上次安装目录。自定义选择的普通文件夹作为父目录，安装器会在其中新建“轻阅 Markdown”专属子目录；重新选择已验证归属的上次安装目录时则原位复用，不会嵌套同名子目录。确认页始终显示最终位置，避免程序文件与原有资料混放。安装、升级和卸载不使用通配符或递归目录删除；跨盘安装保留旧位置供用户自行确认，卸载仅在专属目录标识校验通过后删除精确的程序文件名，用户原有文件、轻阅在安装目录内新建的文档和非空父目录始终保留。桌面快捷方式和 Markdown 文件关联保持不变。安装界面的背景、图标和全部图文源文件设有 800 KB 构建硬限制，不引入视频、浏览器或大型动画运行库。

确认页对常见安装路径使用紧凑的单行路径框，较长路径才扩展为多行。再次运行安装器时，会优先从有效的用户级安装记录、系统级安装记录及旧版记录中定位已安装目录；只有该目录的主程序和专属标记均通过校验才会自动选用，否则退回安全默认路径。点击“自定义安装”时，选择器也会直接定位到这个已验证的安装目录。
...
- 新增文档工具：Markdown 检查、文件夹全文搜索、明确勾选后的批量替换预览、安全中心、自定义模板和隐私诊断导出。
- 保存冲突提供“磁盘版本／当前编辑”并排对比，可保留编辑、采用磁盘版本、另存或手动合并，不自动覆盖。
- 新增全宽“仅编辑”布局，关闭预览时暂停实时渲染；`Ctrl/⌘ + E` 切换完整预览，顶部“恢复预览”可回到上次分栏方向。
- Windows 安装、升级与卸载发布门禁进一步覆盖真实卸载登记和用户文件保留；未知旧卸载器不删除、不改名。
...
- 单实例打开文件和未保存修改保护；退出时可选择保存、不保存或取消，只有保存成功后才会关闭窗口。
